...
def fetch_scheduler(optimizer):
    if SCHEDULER == "ReduceLROnPlateau":
        scheduler = ReduceLROnPlateau(
            optimizer, mode="min", factor=factor, patience=patience, verbose=True, eps=eps
        )
    ...
    def __init__(
        ...
        if loss_module == "arcface":
            self.final = ArcMarginProduct(
                final_in_features, n_classes, s=s, m=margin, easy_margin=False, ls_eps=ls_eps
            )
...
def get_stratified_kfold(df, splits=12, seed=42):
    df["enc_label_group"] = LabelEncoder().fit_transform(df["label_group"])
    skf = StratifiedKFold(n_splits=splits, random_state=seed, shuffle=True)
    folds = [-1 for _ in range(len(df))]
    for fold, (_, tst_index) in enumerate(skf.split(df, df["enc_label_group"])):
        for ix in tst_index:
            folds[ix] = fold
...
data = pd.concat([data, get_stratified_kfold(data)], axis=1)
if DEBUG:
    data = data.sample(1000)

data["filepath"] = data["image"].apply(
    lambda x: os.path.join("../input/shopee-product-matching/", "train_images", x)
)
...
def run(fold):
    ...
    for epoch in range(EPOCHS):
        train_loss = train_fn(
            train_loader, model, criterion, optimizer, device, scheduler=scheduler, epoch=epoch
        )
        ...
        if valid_loss.avg < best_loss:
            ...
            torch.save(
                model.state_dict(), f"fold{fold}_{model_name}_IMG_SIZE_{DIM[0]}_{loss_module}.bin"
            )
            print("best model found for epoch {}".format(epoch))


for fold in range(0, 12, 3):
    print(f"Start trainig fold {fold}")
    run(fold)
